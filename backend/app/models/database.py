...
if not USE_MONGODB:
    ...
    from .mongodb_simple import init_mongodb, mongodb_db
    print("🔄 Using MongoDB database configuration")
    engine = None
    SessionLocal = None
    Base = None
    # Define empty classes for MongoDB mode
    Company = None
    Shipment = None
    APIKey = None
    CustomRoute = None
...
def get_db():
    ...
    if USE_MONGODB:
        # For MongoDB, return the database instance
        try:
            yield mongodb_db
        finally:
            pass  # MongoDB connections are managed by the driver
    else:
        # SQLAlchemy session
        db = SessionLocal()
        try:
